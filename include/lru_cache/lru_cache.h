 ...
 *                                                                         *
 *                                                                         *
 *  adapted to pthread by Pawel Marciniak                                  *                                      *
 ***************************************************************************/
...
#define SCOPED_MUTEX  scoped_lock lock(&this->_mutex);
//#define MUTEX_LOCK  pthread_mutex_lock(this->_mutex);
//#define MUTEX_UNLOCK  pthread_mutex_unlock(this->_mutex);
...
//		boost::mutex _mutex;
	...
	public:
		...
		inline void insert( const Key &key, const Data &data ) {
			SCOPED_MUTEX;
			//cout << "lru insert size: " << _curr_size << " max: " << _max_size << endl; 
			...
			//cout << "done lru insert size: " << _curr_size << " max: " << _max_size << endl;
		}
	...
	private:
		...
		inline void _remove( const Map_Iter &miter ) {
		    //cout << "lru remove" << _curr_size << " max" << _max_size << endl;
		    //lounch client action 
		    if (onRemove) onRemove(miter->second->second);
